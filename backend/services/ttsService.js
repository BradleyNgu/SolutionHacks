...
class TTSService {
  ...
  async textToSpeech(text, options = {}) {
    ...
    try {
      return await this.providers[provider](text, options);
    } catch (error) {
  ...
  /**
   * Browser-based TTS (returns SSML and instructions for client-side)
  ...
  async browserTTS(text, options = {}) {
    return {
      ...
      text: text,
      voice: {
        name: options.voiceName || 'default',
  ...
  /**
   * Web Speech API compatible format
   * @param {string} text - Text to convert
   * @param {Object} options - Voice options
  ...
  async googleCloudTTS(text, options = {}) {
    ...
    const request = {
      ...
      audioConfig: {
        ...
        speakingRate: options.rate || config.tts.audioConfig.speakingRate,
        pitch: options.pitch || config.tts.audioConfig.pitch,
